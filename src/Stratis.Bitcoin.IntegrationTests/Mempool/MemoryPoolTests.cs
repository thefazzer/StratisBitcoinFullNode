    ...
    public class MemoryPoolTests
    {
        ...
        public void MempoolSyncTransactions()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                Parallel.ForEach(trxs, options, transaction => { stratisNodeSync.Broadcast(transaction); });
        ...
        public void MineBlocksBlockOrphanedAfterReorgTxsReturnedToMempool()
        {
            ...
            NodeBuilder builder = NodeBuilder.Create(this);
            CoreNode node1 = builder.CreateStratisPowNode(KnownNetworks.RegTest).NotInIBD().WithWallet();
            CoreNode node2 = builder.CreateStratisPowNode(KnownNetworks.RegTest).NotInIBD().WithWallet();
            ...
            node1.FullNode.MempoolManager().Validator.AcceptToMemoryPool(mempoolValidationState, transaction).Result.Should().BeTrue();
            TestHelper.WaitLoop(() => node1.FullNode.MempoolManager().MempoolSize().Result == 1);
            node1.FullNode.MempoolManager().GetMempoolAsync().Result.Should().Contain(transaction.GetHash());
            ...
            // New mined block contains this transaction from the orphaned blo.
            TestHelper.MineBlocks(node1, 1);
            node1.FullNode.Chain.Tip.Block.Transactions.Contains(transaction);
        }
    }
}
